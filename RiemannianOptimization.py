...
class RiemannianOptimization():
    ...
    def __init__(self, rank=10, max_iter=100, tol=1e-5, reg_coef=None):
    ...
    def projection(self, Y1, Y2):
        ...
        proj = scp.csr_matrix((data, (rows, cols)), shape=self._A.shape)
    ...
    def create_factors(self, X_svd):
        """
        Create factors from svd representatine of X.
        Args:
    ...
    def Initialization(self, A):
        ...
        self._Omega = self._A.nonzero()
        
        
        diag = np.random.uniform(0, 1, self._rank)
        diag = np.sort(diag)[::-1]
        
        S1 = np.diag(diag)
        U1 = np.random.uniform(0, 1, (self._m, self._rank))
        V1 = np.random.uniform(0, 1, (self._n, self._rank))
        
        Qu1, Ru1 = np.linalg.qr(U1)
        Qv1, Rv1 = np.linalg.qr(V1)
        X = [Qu1, S1, Qv1]
        
        Y1, Y2 = self.create_factors(X)
    ...
    def line_search(self, X, Rw, grad, con_grad):
        ...
        while True:
            ...
            Y1, Y2 = self.create_factors(X_new)
            f_new = 0.5*(linalg.norm(self.projection(Y1, Y2) - self._A)**2)
    ...
    def fit(self, A, trace=False, debug_mode=False, random_state=123):
        """
        ...
        X_old, X_new, xi_old, eta_old = self.Initialization(A)
        
        Y1, Y2 = self.create_factors(X_new)
        #Y1Y2t_old = Y1.dot(Y2.T)
        Xw = self.projection(Y1, Y2)
        R = Xw - self._A
        ...
        while True:
            ...
            xi_new_norm = self.scalar_product(xi_new, xi_new)**2
            
            if xi_new_norm <= self._tol:
            ...
            #Y1Y2t = Y1.dot(Y2.T)
            #diff_norm = np.linalg.norm(Y1Y2t - Y1Y2t_old) ** 2
            
            if trace:
                
                f = 0.5*(linalg.norm(R, ord='fro')**2)
                
                history['residuals'].append(f)
                
                history['diff_norm'].append(xi_new_norm)
            ...
            if debug_mode:
                print("Number of iteration: ", iters, " ", "xi_norm^2: ", xi_new_norm)
       
        
        self._history = history
        
        self._factor_1 = X_new[0]
        self._factor_2 = X_new[1]
        self._factor_3 = X_new[2]
    ...
    def get_factors(self):
        """
        Returns U, S, Vh - factorization of matrix, used in fit method
        """
        return self._factor_1, self._factor_2, self._factor_3
        
    def predict(self):
        """
        Returns the product of factorization matrices B and B: AB^T
        """
        return (self._factor_1).dot(self._factor_2).dot(self._factor_3.T)
